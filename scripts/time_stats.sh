...
#脚本功能，遍历待处理文件夹，然后使用py脚本从去除异常值后的时间文件内统计平均信息,并将输出结果移动到相应文件夹
...
#存放去除异常值结果的文件夹路径
result_dir=${script_dir}/../result_handle/result_stats
...
for graph_dir in "${filepath}"/graph*; do
    ...
    for degree_dir in "${graph_dir}"/d*; do
        ...
        result_stats="${result_dir}/${graph_name}/${degree_name}"
        mkdir -p "$result_stats"
        ...
        for data_file in "${degree_dir}"/*.txt; do
            ...
            file_to_mv=${filepath}/${graph_name}/${degree_name}/s_${filename}
            # echo "file_to_mv is $file_to_mv"

            #执行python时间提取脚本,该脚本会在当前路径下创建e_filename.txt文件
            # 验证并执行脚本
            if [[ -f "$fullpath" ]]; then
                python3 time_stats.py $fullpath
            else
