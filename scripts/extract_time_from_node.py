...
def process_graph_folder(folder_path, program_name):
    """处理graph文件夹并输出排序后的时间值"""
    dirs = []
    filename = f"last_s_rmo_e_{program_name}.txt"
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("使用方法: python script.py <graph文件夹路径> <程序名称>")
        sys.exit(1)
    ...
    process_graph_folder(input_folder, program_name)