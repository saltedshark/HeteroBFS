/***
 * 函数功能：根据节点数与平均度从seq，omp，cuda中选择最佳性能的程序进行执行
 * 该代码简单将分离的seq，omp和cuda放到同一个文件，增加一个选择模块
 */

// src/select_seq_omp_cuda.cu
// ================================
...
#include <omp.h>
#define OPTIMIZE_LEVEL 0 //保持与omp的一致
//优化等级
//分0,1,2
//0 for 原始版本
...
//选择调度模块，该模块传入节点数与边数，自动计算平均度，然后根据平均度与节点数两个信息选择程序，传回程序编号
//1 for seq
//2 for omp
//3 for cuda-thrust
int select(int no_of_nodes, int edge_list_size);
...
int main(int argc, char** argv){
    ...
    int num_omp_threads = op.getOptionInt("omp_threads");//for omp
    ...
    int proc = select(no_of_nodes, edge_list_size);
    printf("select : %d\n", proc);//用于测试

    auto end_t = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(end_t - start_t);
    double select_time = double(duration.count()) * microseconds::period::num / microseconds::period::den;
    //输出选择模块的时间
...
void opinit(OptionParser &op){
    ...
    op.addOption("omp_threads",OPT_INT, "16", "specify number of openmp threads", 't');
