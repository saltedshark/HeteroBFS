...
__global__ void bfs_edge_kernel(uint32_t *offsets, uint32_t *edges,int *current_queue, int *current_size,
    int *next_queue, int *next_size, int *visited, int* d_cost, int total_edges)
{
    ...
    //这个是保证的吗？？
    //没保证，所以每次启动kernel之前都需要对当前层邻居排序，既方便找边又方便算total_edges
    ...
    // 原子操作检查并标记 visited
    //先将visited[neighbor]修改为true,表示已访问,然后再返回修改前的值
    bool old_visited = atomicExch(&visited[neighbor], 1);
    //这个操作只支持int或uint，不支持bool或char，所以要么转换为int要么使用位压缩
    if (!old_visited) {  // 未被访问过
        int src_cost = d_cost[src_node];
        int expected = -1;
...
void cuda_bfs_edge(int no_of_nodes, int source, uint32_t *&h_offsets,
    uint32_t *&d_offsets, uint32_t *&d_edges, int *&d_cost, int *&d_visited,
...
void cuda_bfs_edge_uvm(OptionParser &op, int no_of_nodes, int source, 
    ...
    int *&next_queue, int *&next_size, int *&visited, int *&cost, 
...
// 查找未访问节点的辅助函数
int find_unvisited_vertex(int* d_visited, int num_vertices) {
...
float BFSGraph(int no_of_nodes, int edge_list_size, uint32_t *&h_offsets, uint32_t *&h_edges) 
{
    ...
    int *d_visited= nullptr;  // 位掩码标记已访问节点
    int *d_cost = nullptr;
    ...
    CUDA_SAFE_CALL(cudaMalloc(&d_visited, no_of_nodes * sizeof(int)));
    ...
    while(true){
        //设备端查找未访问节点
        int32_t start_vertex = find_unvisited_vertex(d_visited, no_of_nodes);
        // printf("start node is %d\n", start_vertex);
        if(start_vertex == -1) break;//全访问过
        //然后调用cuda_bfs执行
        cuda_bfs_edge(no_of_nodes, start_vertex, h_offsets,
            d_offsets, d_edges, d_cost, d_visited,
...
float BFSGraphUnifiedMemory(OptionParser &op, int no_of_nodes, int edge_list_size, uint32_t *&offsets, uint32_t *&edges) {
    ...
    //分配状态相关数组及cost并初始化
    //原来设备端指针全改uvm
    uint32_t *graph_offsets = nullptr;//设备端使用
    uint32_t *graph_edges = nullptr;//设备端使用
    int *current_queue = nullptr;
    int *next_queue = nullptr;
    int *current_size = nullptr;
    int *next_size = nullptr; // 队列大小（设备端）
    int *visited = nullptr; //都在设备使用
    int *cost = nullptr;//都在设备使用，执行完才需要拷贝回cpu端
    

    //计算大小，防止后面重复计算，且不易错
    const size_t offsets_size = sizeof(uint32_t) * (no_of_nodes + 1);
    const size_t edges_size = sizeof(uint32_t) * edge_list_size;
    const size_t visited_size = sizeof(int) * no_of_nodes;
    const size_t cost_size = sizeof(int) * no_of_nodes;
    const size_t queue_size = sizeof(int) * no_of_nodes;
    ...
    if (uvm || uvm_advise || uvm_prefetch || uvm_prefetch_advise) {
        ...
        //cost
        checkCudaErrors(cudaMallocManaged(&cost, cost_size));
    }
    ...
    if (uvm) {
        ...
        checkCudaErrors(cudaMemPrefetchAsync(cost, cost_size, device));
        checkCudaErrors(cudaMemPrefetchAsync(current_queue, queue_size, device));
        checkCudaErrors(cudaMemPrefetchAsync(next_queue, queue_size, device));
    } else if (uvm_advise) {
        ...
        checkCudaErrors(cudaMemAdvise(visited, visited_size, cudaMemAdviseSetPreferredLocation, device));
        checkCudaErrors(cudaMemAdvise(cost, cost_size, cudaMemAdviseSetPreferredLocation, device));
        checkCudaErrors(cudaMemAdvise(current_queue, queue_size, cudaMemAdviseSetPreferredLocation, device));
        checkCudaErrors(cudaMemAdvise(next_queue, queue_size, cudaMemAdviseSetPreferredLocation, device));
    ...
    cudaStreamCreate(&s2);
    cudaStreamCreate(&s3);
    cudaMemsetAsync(current_queue, -1, queue_size, s1);
    cudaMemsetAsync(next_queue, -1, queue_size, s2);
    cudaMemsetAsync(visited, 0, visited_size, s3);
    cudaMemsetAsync(cost, -1, cost_size);//使用默认流相当于s0
    ...
    while(true){
        int32_t start_vertex = find_unvisited_vertex(visited, no_of_nodes);
        if(start_vertex == -1) break;//全访问过
        //然后调用cuda_bfs执行
        // printf("visite node : %d\n", i);
        cuda_bfs_edge_uvm(op, no_of_nodes, start_vertex, offsets, 
    ...
    if (uvm) {
        ...
        checkCudaErrors(cudaMemAdvise(cost, cost_size, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId));
        checkCudaErrors(cudaMemAdvise(cost, cost_size, cudaMemAdviseSetReadMostly, cudaCpuDeviceId));
    } else if (uvm_prefetch) {
        checkCudaErrors(cudaMemPrefetchAsync(cost, cost_size, cudaCpuDeviceId));
    } else if (uvm_prefetch_advise) {
        checkCudaErrors(cudaMemAdvise(cost, cost_size, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId));
        checkCudaErrors(cudaMemAdvise(cost, cost_size, cudaMemAdviseSetReadMostly, cudaCpuDeviceId));
        checkCudaErrors(cudaMemPrefetchAsync(cost, cost_size, cudaCpuDeviceId));
        ...
        *current_size = 1;
        ...
        while (*current_size > 0) {
            ...
            *next_size = 0;
            ...
            // 移除主机端排序操作，改为设备端排序
            // 设备端直接排序current_queue
            thrust::sort(thrust::device, current_queue, current_queue + *current_size, 
                CompareOffsets(graph_offsets));
...
//设置设备端数组状态的内核函数
__global__ void setValueKernel(int* visited, int* cost, int idx) {
...
void cuda_bfs_edge( int no_of_nodes, int source, uint32_t *&h_offsets,
    ...
    double &kernel_time, double &transfer_time, int &k)
    {
        ...
        //设备端直接设置d_visited、d_cost
        // 启动 1 个线程块，内含 1 个线程
        cudaEventRecord(tstart, 0);
        setValueKernel<<<1, 1>>>(d_visited, d_cost, source);
        cudaEventRecord(tstop, 0);
        cudaEventSynchronize(tstop);
        cudaEventElapsedTime(&elapsedTime, tstart, tstop);
        transfer_time += elapsedTime * 1.e-3;
