...
void inner_bfs(cugraph_resource_handle_t* handle,cugraph_graph_t* graph,
    int32_t* d_distances, int32_t d_visited, size_t num_vertices,double &transfer_time, size_t &cnt);
    ...
    double &transfer_time, double &kernel_time, size_t &cnt)
{
    ...
    while(true) {
        // 查找首个未访问节点（设备端操作）
        int32_t start_vertex = find_unvisited_vertex(d_visited, num_vertices);
        ...
        //当前遍历结果
        const int32_t *bfs_distances = static_cast<const int32_t*>(cugraph_type_erased_device_array_view_pointer(current_dist_view));
        // 合并距离到全局数组（使用thrust操作原始指针）
        thrust::device_ptr<const int32_t> bfs_ptr(bfs_distances);
        thrust::device_ptr<int32_t> dist_ptr(d_distances);
        thrust::transform(
            thrust::device,
            bfs_ptr,
            bfs_ptr + num_vertices,
            dist_ptr,
            dist_ptr,
            [] __device__ (int bfs_d, int global_d) {
                //cugraph会将当前遍历结果不可达节点的相关距离数组设置为INT_MAX
                //当前bfs遍历结果有效时保留当前结果，无效时保留全局结果
                return (bfs_d != INT_MAX) ? bfs_d : global_d;  // 保留cuGraph的INF处理
            }
        );
              
        // 更新已访问状态
        thrust::device_ptr<bool> visited_ptr(d_visited);
        thrust::transform(
            thrust::device,
            bfs_ptr,
            bfs_ptr + num_vertices,
            visited_ptr,
            visited_ptr,
            [] __device__ (int d, bool visited) {
                //根据当前层遍历结果，设置visited状态
                //若遍历结果无效时（即不可达），保留原状态
                //遍历结果有效时，设置已访问
                return visited || (d != INT_MAX);  // 保持cuGraph的原始逻辑
            }
        );
...
void run_bfs(const uint32_t* h_offsets, const uint32_t* h_edges,
    ...
    int32_t *d_distances;
    bool *d_visited;//增加一个visited数组，用于未访问控制
    ...
    CUDA_CHECK(cudaMemset(d_visited, 0, num_vertices * sizeof(bool)));
    // printf("距离数组初始化完毕\n");
    ...
    // 执行分层BFS
    inner_bfs(handle, graph, d_distances, d_visited, num_vertices,
